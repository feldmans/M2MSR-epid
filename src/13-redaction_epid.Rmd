...
Je fais une première étape de datamanagement "basique" pour mettre les variables au bon format (dates, numérique et facteur), m'assurer que le numero de patient est en charactère, créer une variable SWAN en 0/1 et une autre en True/False selon les différents modèles utilisés par la suite. Je change également les "" en nocatégorie pour la variable catégorie ou en NA pour les dates. Je choisis No comme référence pour la variable CA

## Vérification de la durée de suivi
```{r}
#Duree de suivi :
...
Pour cela je cree d'abord la variable temps de suivi comme la différence entre la date des dernieres nouvelles et la date d'admission dans l'étude.  Puis je modifie cette variable telle que si le temps de suivi est supérieur à 30 jours alors je le ramène à 30 jours. Et la variable décès à 30 jours est modifiée comme ceci à partir de la variable décès : si le décès est survenu après 30 jours, alors la variable décès à 30 jours vaut 0.  
...
La base de données est globalement de bonnes qualité, en effet aucun sujet n'a plus de 4 valeurs manquantes (sur les 63 variables).
...
## Analyse descriptive préliminaire afin de repérer les incohérences
...
Pour détecter les valeurs aberrantes je dispose de plusieurs méthodes. Je regarde le tableau descriptif en calculant pour chaque variable quantitative la moyenne, l'intervalle interquartile et surtout le range (tableau non présenté). Je croise également les variables fréquence cardiaque, pression artérielle et fréquence respiratoire à la recherche d'incohérence. Enfin je regarde la distribution des variables ce qui me permet de réperérer éventuellement d'autres incohérences. (voir annexe) Par exemple nous voyons un pic à 0 pour la variable WTKILO1 (nous l'avions déjà vu grâce au tableau descriptif, mais c'est un deuxième filet de sécurité). 
...
Les valeurs de natrémie, de kaliémie, de fréquence respiratoire, fréquence cardiaque, tension artéirelle et bilirubine semblent plausibles.  
...
## Description des valeurs manquantes (valeurs aberrantes transformées en valeurs manquantes)
...
Il n'y a pas de valeurs manquantes concernant la variable explicative d'intérêt Swan ganz
...
Table. variables qui ont été imputées, avant imputation
```{r}
#apres imputation
kable(do.call(rbind, lapply(names(nacol[nacol>0]), describe_all, d)))
```

Table. variables qui ont été imputées, après imputation

La distribution des variables a l'air semblable avant et après imputation.


# Description de toutes les variables après imputation
```{r}
...
Je regarde les différences entre le groupe des patients cathétérisés et le groupe des patients non cathétérisés. Je ne réalise pas de test car une petite différence numériquement peut être significative du fait de la taille de l'échantillon. 
...
Les 10 variables liées uniquement au cathétérisme ne sont pas prises en comptes dans le score :
...
#clogit(DTH30~SWAN + cluster(paire), method="efron", data=d2.app) #marche avec efron
...
coxph(Surv(rep(1, nrow(d2.app)),DTH30)~SWAN + cluster(paire), data=d2.app) #marche mais est-ce vraiment la même chose?
#glm(DEATH~SWAN, data=d2.app, family="binomial")
glm(DTH30~SWAN, data=d2.app, family="binomial")
#glmer(DEATH~SWAN + (1|paire), data=d2.app, family = "binomial")
glmer(DTH30~SWAN + (1|paire), data=d2.app, family = "binomial")
```
...
#Description des variables avant imputation
```{r, eval=FALSE}
